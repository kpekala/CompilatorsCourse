...
_lr_productions = [
  ...
  ('program -> ins_opt','program',1,'p_program','Mparser.py',29),
  ('ins_opt -> ins','ins_opt',1,'p_ins_opt','Mparser.py',34),
  ('ins -> ins in','ins',2,'p_ins','Mparser.py',39),
  ('ins -> in','ins',1,'p_ins','Mparser.py',40),
  ('in -> ;','in',1,'p_in_empty','Mparser.py',45),
  ('in -> { ins }','in',3,'p_ins_group','Mparser.py',49),
  ('in -> ID = expr ;','in',4,'p_assign','Mparser.py',53),
  ('in -> ID ASSIGNADD expr ;','in',4,'p_assign','Mparser.py',54),
  ('in -> ID ASSIGNMINUS expr ;','in',4,'p_assign','Mparser.py',55),
  ('in -> ID ASSIGNMUL expr ;','in',4,'p_assign','Mparser.py',56),
  ('in -> ID ASSIGNDIV expr ;','in',4,'p_assign','Mparser.py',57),
  ('expr -> expr + expr','expr',3,'p_expr_binop','Mparser.py',62),
  ('expr -> expr - expr','expr',3,'p_expr_binop','Mparser.py',63),
  ('expr -> expr * expr','expr',3,'p_expr_binop','Mparser.py',64),
  ('expr -> expr / expr','expr',3,'p_expr_binop','Mparser.py',65),
  ('expr -> expr DOTADD expr','expr',3,'p_expr_binop','Mparser.py',66),
  ('expr -> expr DOTMINUS expr','expr',3,'p_expr_binop','Mparser.py',67),
  ('expr -> expr DOTTIMES expr','expr',3,'p_expr_binop','Mparser.py',68),
  ('expr -> expr DOTDIVIDE expr','expr',3,'p_expr_binop','Mparser.py',69),
  ('expr -> - expr','expr',2,'p_u_minus','Mparser.py',73),
  ('expr -> FLOATNUM','expr',1,'p_expr_lit','Mparser.py',77),
  ('expr -> NUMBER','expr',1,'p_expr_lit','Mparser.py',78),
  ('expr -> ID','expr',1,'p_expr_id','Mparser.py',83),
  ('expr -> STR','expr',1,'p_expr_str','Mparser.py',88),
  ('expr -> ( expr )','expr',3,'p_expr_grp','Mparser.py',94),
  ("expr -> expr '",'expr',2,'p_transpose','Mparser.py',99),
  ...
  ('expr -> [ lists ]','expr',3,'p_literal_matrix','Mparser.py',114),
  ('lists -> list','lists',1,'p_lists','Mparser.py',118),
  ('lists -> lists , list','lists',3,'p_lists','Mparser.py',119),
  ('list -> [ seq ]','list',3,'p_list','Mparser.py',124),
  ('seq -> expr','seq',1,'p_seq','Mparser.py',129),
  ('seq -> seq , expr','seq',3,'p_seq','Mparser.py',130),
  ('fun -> ZEROS','fun',1,'p_fun','Mparser.py',135),
  ('fun -> EYE','fun',1,'p_fun','Mparser.py',136),
  ('fun -> ONES','fun',1,'p_fun','Mparser.py',137),
  ('expr -> fun ( expr )','expr',4,'p_funcall','Mparser.py',141),
  ('in -> WHILE ( cond ) in','in',5,'p_while','Mparser.py',147),
  ('in -> FOR ID = expr : expr in','in',7,'p_for','Mparser.py',153),
  ('in -> IF ( cond ) in','in',5,'p_if','Mparser.py',158),
  ('in -> IF ( cond ) in ELSE in','in',7,'p_if','Mparser.py',159),
  ('in -> BREAK ;','in',2,'p_control','Mparser.py',164),
  ('in -> CONTINUE ;','in',2,'p_control','Mparser.py',165),
  ('in -> RETURN expr ;','in',3,'p_control','Mparser.py',166),
  ('in -> PRINT seq ;','in',3,'p_print','Mparser.py',176),
  ('in -> ID list = expr','in',4,'p_access','Mparser.py',181),
]
